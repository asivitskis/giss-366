...
> **[INSTRUCTOR NOTE — before class]** Create one workspace per student (or have students create their own, named `[lastname]_giss366`, to avoid collisions) on your shared instance. Decide in advance whether students get individual GeoServer logins or share one admin login and self-police by using unique workspace names — the latter is simpler for a first-time GeoServer admin and is what these instructions assume. See the "Instructor Lab Prep" notes for setup steps.
