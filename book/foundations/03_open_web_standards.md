...
Once a layer is published, GeoServer automatically generates that `GetCapabilities` document from the demo above, and any WMS/WFS-aware client, whether GeoServer's own preview, QGIS, a Leaflet map, or ArcGIS Online, can discover and consume it without any custom integration work. That's the interoperability payoff from 2.1, in practice.
...
> **A caution worth naming:** part of the pressure toward cloud-native and managed platforms isn't just cost or convenience, it's security. GeoServer runs on a Java web container (Tomcat), and self-hosted instances have been a real target for exploits in recent years; keeping one patched takes ongoing, specialized attention that not every organization can staff. That's a legitimate reason the market is shifting, and it's worth watching, not dismissing. But the protocol still outlives the backend: an agency that migrates away from GeoServer toward ArcGIS Enterprise, a managed host, or something else entirely will very likely still expose WMS/WFS, because that's the compatibility layer everyone else's software expects. And plenty of deep, high-stakes GIS infrastructure (think USGS, NOAA) isn't migrating off GeoServer anytime soon. Read the "is GeoServer dying" discourse with that in mind: the backend is genuinely in flux, but the standard you're learning this week is the part likeliest to still be there in five years.

## Consuming Services in ArcGIS Online

You already spent last week inside ArcGIS Online's Map Viewer publishing your own hosted feature layer. Interoperability isn't only a GeoServer feature, though; it flows both directions. ArcGIS Online can consume external WMS and WFS endpoints directly: from a map in AGOL, **Add → Layer from Web**, choose the service type, and paste in the GeoServer endpoint URL. AGOL parses the same `GetCapabilities` document any other client would, lists the available layers, and lets you add one to your map with no export, no conversion, and no Esri-specific format required on the GeoServer side.

This is the dual-stack philosophy from Week 1 in action: a layer published once from a self-hosted, open-source server can be consumed by a proprietary platform, and vice versa (AGOL can also publish its own WMS/WFS endpoints from a hosted feature layer, consumable by GeoServer, QGIS, or a hand-built Leaflet map). Neither stack has to "win"; the standard is the thing that lets them coexist on the same project.
...
2. If GeoServer requires a server that's always running, and Week 6's cloud-native formats need only static storage, what's the tradeoff an organization is actually making when it picks one over the other? (Revisit this one after Week 6 and see if your answer changes.)

---

### In-Class Exercise: Three Mystery Layers

Small groups, one URL each. In ArcGIS Online: **Add → Add Layer from URL**, paste the URL, let AGOL auto-detect the service type, then **Next** and pick a layer. Only toggle "use as basemap" if you actually want it as one. For each layer, groups report back on: what AGOL named the layer, whether clicking a feature returns real attribute data or just an image, what the data shows and who'd want it, and what would happen if someone tried to open the same link in a different mapping program.

1. **WMS** — `https://www.mrlc.gov/geoserver/ows?service=WMS&version=1.3.0&request=GetCapabilities`
2. **WFS** — `https://www.mrlc.gov/geoserver/ows?service=WFS&acceptversions=2.0.0&request=GetCapabilities`
3. **WMTS** — `https://www.mrlc.gov/geoserver/gwc/service/wmts?service=WMTS&acceptVersions=1.0.0&request=GetCapabilities`
...
## Optional Prep: Before Thursday

None of what follows is graded, and none of it will be assumed knowledge in Lab 2. If you want more comfortable footing before Thursday's hands-on publish sprint, pick whichever of these feels useful.

### Exercise 1: Explore the MRLC GeoServer Web Interface (10–15 min)

Visit the [MRLC GeoServer landing page](https://www.mrlc.gov/geoserver/web/) and browse the layer list. Click Layer Preview on a layer you didn't touch in class and open it in OpenLayers. What does it show? Is it the same kind of data you tested in the Explore activity, or something different? This is the same style of public interface, though read-only, that you'll be navigating pieces of on our class instance Thursday.

### Exercise 2: GetCapabilities Scavenger Hunt (10 min)

On the Layer Preview page, pick a layer you didn't use in class and note its exact name (it looks like `workspace:layername`). Then open the WMS GetCapabilities URL from the worksheet. Clicking WMS on the landing page returns one long document listing every layer on the server, not just one, so use your browser's find function (Ctrl+F, or Cmd+F on Mac) and search for the layer name you wrote down. That drops you into the `<Layer>` block for your specific layer, where you'll find its `<Title>` and its `<BoundingBox>`. The list of supported output formats, by contrast, isn't inside each layer's block; it's listed once near the top of the document under `<GetMap><Format>`, and applies to every layer on the server.

Stretch goal: edit the URL, swap `GetCapabilities` for `GetMap`, add a bounding box parameter, and watch the response change from a menu to an actual picture. You'll be reading this exact document structure, generated by our own server this time, during Thursday's read-along.

### Exercise 3: Scout Your Lab 2 Dataset (10–15 min)

If you haven't already, browse the NM RGIS Clearinghouse and download a small vector dataset for Grant County or the Gila as a shapefile. Trails, fire perimeters, or public land boundaries are good choices. Note what actually downloads: a shapefile is really several files (`.shp`, `.shx`, `.dbf`, `.prj` at minimum). Can you find all of them once you unzip it? Thursday's Store step expects exactly this file set, so finding it now means less digging mid-lab.

### Exercise 4: Check Your Dataset's Coordinate System (5 min, only if you did Exercise 3)

Open your downloaded shapefile in QGIS, or any GIS software you have, and find its coordinate reference system in the layer properties. Jot down what it is. You'll need to set the Declared SRS to match it on Thursday.

### Exercise 5: GeoServer's Own "Getting Started" Page (10 min)

Skim [GeoServer's Getting Started documentation](https://docs.geoserver.org/latest/en/user/gettingstarted/index.html). You'll see the same store, layer, style, and workspace vocabulary from tonight's lecture, described from GeoServer's own point of view.
